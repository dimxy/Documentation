...
The `Assets` Crypto Conditions smart contract provides basic distributed exchange (DEX) functionality for trading tokens. The functionality is facilitated by utxo technology. Tokens can be created on any asset chain where the [ac_cc](../installations/asset-chain-parameters.html#ac-cc) is enabled. 

To use CC Assets features you need first to create tokens. For token creation and management use CC Tokens smart contract rpc methods (../basic-docs/cryptoconditions/cc-tokens.html).

### Requirements

For using CC Assets functionality it is needed to create tokens with CC Tokens contract. 

### CC Assets Workflow

CC Assets allows to buy or sell tokens for ordinary coins.

The workflow to sell tokens:
* a token owner places a new ask with `tokenask` method stating he wants to sell a certain amount of tokens for the appointed price. Assets contract creates a new token ask order and that asked amount of tokens is locked on CC Assets global address.
* a buyer who want to buy some amount of tokens in the ask for the proposed price runs `tokenfillask` method. The purchased token amount moves from the global cc address to the buyer token CC address and at the same time the required amount of coins moves from the buyer to seller's address. Such purchases might be repeated while there is a remainder of tokens in the ask order.  
* at any time the order creator can cancel it with `tokencancelask` method. The token remainder would return to his token cc address.  

The workflow to buy tokens
* a user who wants to buy tokens owner places a new bid with `tokenbid` method stating he wants to buy a certain amount of tokens for the appointed price. Assets contract creates a new token bid order and that amount of coins is locked on CC Assets global address.
